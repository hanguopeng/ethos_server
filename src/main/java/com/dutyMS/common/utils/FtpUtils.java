...
//                System.exit(1);
     ...
     */
    public boolean renameFile(String oldFileName,String newFileName,String ftpPath) {
        try {
    ...
    public boolean upload(byte[] bytes,String ftpFileName, String substring, String ftpDirectory) {
        if (!ftpClient.isConnected()) {
            return false;
        }
        boolean flag = false;
        if (ftpClient != null) {
            ...
            try {
                ...
                ftpClient.setFileType(FTPClient.BINARY_FILE_TYPE);
                // 上传
                flag = ftpClient.storeFile(new String(ftpFileName.getBytes("GBK"), "iso-8859-1"), sbs);
            }catch (Exception e) {
                this.close();
                e.printStackTrace();
                return false;
    ...
    public static void main(String[] args) {
        FtpUtils f = new FtpUtils("172.16.5.186", 21, "ftpadmin", "jingming");
        try {
            if(f.open()) {
                String fileName = "测试2.txt";
                //上传
                f.upload("d:/text.txt", fileName, "/");
    ...
    public byte[] downLoadFile(String filePath) {
        String cacheFilePath = "/cache_"+filePath;
        byte [] objFile = null;
        if(get(filePath,cacheFilePath))
        {
            File localFile = new File(cacheFilePath);
